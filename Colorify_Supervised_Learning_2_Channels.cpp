...
// ----------------------------------------------------------------------------------------
matrix<rgb_pixel> concact_channels_lr(const matrix<gray_pixel>& gray_image, const matrix<uint16_t>& ab_image) {
    ...
    for (long r = 0; r < lab_image.nr(); ++r) {
        for (long c = 0; c < lab_image.nc(); ++c) {            
            ...
            lab_image(r, c).a = static_cast<double>(ab_image(r, c) >> 4) * (255.0 / 15.0);
            lab_image(r, c).b = static_cast<double>(ab_image(r, c) & 0xF) * (255.0 / 15.0);
        }
    }
...
matrix<rgb_pixel> concact_channels_hr(const matrix<gray_pixel>& gray_image, std::array<matrix<float>, 2>& ab_image) {
    matrix<lab_pixel> lab_image(gray_image.nr(), gray_image.nc());
    for (long r = 0; r < lab_image.nr(); ++r) {
        for (long c = 0; c < lab_image.nc(); ++c) {
            lab_image(r, c).l = gray_image(r, c);
            lab_image(r, c).a = dequantize(ab_image[0](r, c));
            lab_image(r, c).b = dequantize(ab_image[1](r, c));
        }
    }
    matrix<rgb_pixel> output;
    assign_image(output, lab_image);
...
// ----------------------------------------------------------------------------------------
matrix<rgb_pixel> generate_image_for_lr_display(display_lr_generator_type& net, const matrix<gray_pixel>& src) {
    matrix<gray_pixel> gray_image = src;
    resize_inplace(gray_image, std_image_size);
    matrix<uint16_t> output = net(gray_image);
    matrix<rgb_pixel> rgb_image = concact_channels_lr(gray_image, output);
...
matrix<rgb_pixel> generate_image_for_hr_display(display_hr_generator_type& net, const matrix<gray_pixel>& src) {
    matrix<gray_pixel> gray_image = src;
    resize_inplace(gray_image, std_image_size);
    std::array<matrix<float>, 2> output = net(src);
    matrix<rgb_pixel> rgb_image = concact_channels_hr(src, output), blur;
    scale_image(src.nr(), src.nc(), rgb_image);
...
void randomly_crop_image(
    ...
    crop.output_image = quantize_ab_channels(src_image);
}
void randomly_crop_image(
    ...
    crop.input_image.set_size(lab_image.nr(), lab_image.nc());
    crop.output_image[0].set_size(lab_image.nr(), lab_image.nc());
    crop.output_image[1].set_size(lab_image.nr(), lab_image.nc());
    for (long r = 0; r < lab_image.nr(); ++r) {
        for (long c = 0; c < lab_image.nc(); ++c) {
            crop.input_image(r, c) = lab_image(r, c).l;
            crop.output_image[0](r, c) = quantize(lab_image(r, c).a);
            crop.output_image[1](r, c) = quantize(lab_image(r, c).b);
        }
    }
}
...
int main(int argc, char** argv) try {
    ...
    if (option == "--norm") {
        ...
        const std::vector<file> training_images = dlib::get_files_in_directory_tree(argv[2], dlib::match_endings(".jpeg .jpg"));
        ...
        while (!g_interrupted) {                        
            // We should see that the generated images start looking like samples
            if (++iteration % update_display == 0) {
                ...
                for (auto& image : gen_samples) {
                    src_img = concact_channels_lr(samples[pos_i], labels[pos_i]);
                    rgb_gen = concact_channels_lr(samples[pos_i], image);
        ...
        while (!g_interrupted) {
            ...
            if (++iteration % update_display == 0) {
                ...
                size_t pos_i = 0, max_iter = __min(gen_samples.size(), 4);
                for (auto& image : gen_samples) {
                    src_img = concact_channels_hr(samples[pos_i], labels[pos_i]);
                    rgb_gen = concact_channels_hr(samples[pos_i], image);
                    disp_imgs.push_back(join_rows(src_img, rgb_gen));
                    if (++pos_i >= max_iter) break;
                }
                image_to_save = tile_images(disp_imgs);
        ...
        // Load the model        
        display_lr_generator_type generator;
        if (file_exists("lowres_colorify.dnn")) {            
            deserialize("lowres_colorify.dnn") >> generator;            
        } else {
        ...
        for (auto& i : images) {            
            ...
            dlib::assign_image(display_gray_image, gray_image);

            gen_image = generate_image_for_lr_display(generator, gray_image);
        ...
        // Load the model        
        display_hr_generator_type generator;
        if (file_exists("highres_colorify.dnn")) {
            deserialize("highres_colorify.dnn") >> generator;
        } else {
        ...
        for (auto& i : images) {
            ...
            gen_image = generate_image_for_hr_display(generator, gray_image);
            win.set_title("COLORIFY - Grayscale " + to_string(input_image.nc()) + "x" + to_string(input_image.nr()) + ") | Original | Generated (" + to_string(gen_image.nc()) + "x" + to_string(gen_image.nr()) + ")");
            win.set_image(join_rows(display_gray_image, join_rows(input_image, gen_image)));
            std::cout << i.full_name() << " - Hit enter to process the next image or 'q' to quit";
            char c = std::cin.get();
